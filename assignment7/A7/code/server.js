...
var fs = require('fs');
var dbURL = 'mongodb://34.227.158.181:27017/test';
...
app.post('/uploadBase64', function(req, res) {
  ...
  s3.putObject(params, function(err, data) {
      db.collection("images").insert({intname: "Untitled", url: intname, userID:req.user.local.email, filter:"none", date: new Date().getTime()}, function(err, result) {
          res.end("success");
          console.log(err);
    });
  });
...
app.get("/updateImageTitle", function(req, res){
    ...
    var id = req.query.id;
    db.collection('images').update({ _id: MS.helper.toObjectID(id) }, { $set: { intname: title} }, function (err, result) {
        if (err) {
            console.log(err);
        } else {
            res.send(result);
