   ...
   filterSelect = M.FormSelect.init(elems, {});
    ...
    loadURL("/getUserImages", function(data){
        ...
        if(allImages.length > 0){
            var profilePicSrc = bucketURL.concat(allImages[0].url);
            console.log(profilePicSrc);
            document.getElementById("profilePic").style.backgroundImage = "url(".concat(profilePicSrc, ")");
        }
    });
}
...
function updateImageTitle()
{
    ...
    $.get("/updateImageTitle", {id: img._id, title: imgTitle}, function(data){ //make sure imgTitle is used as the title or else title will show up null on page
        console.log(data);
    });
}
